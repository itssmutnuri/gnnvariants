...
#%% change data to cumulative then Spline smooth the data then changr back to incident
var_prev = variants
var_prev['Week']= pd.to_datetime(var_prev['Week'])
countries_var =  list(set(var_prev.Country))
all_vars =  list(set(var_prev.Variant))
var_prev_N = pd.DataFrame(columns=['pangoLineage', 'country', 'date', 'count', 'total'])

for c in countries_var:
    print(c)
    for v in all_vars:
        var_table_source = var_prev[(var_prev.Variant==v) & (var_prev.Country==c)]
        x = var_table_source.Week
        if x.shape[0]<=1: #Skip missing data or anything which just appears a week
            continue
        var_table_source['Total Sequences'] = var_table_source['Total Sequences'].cumsum() #change data to cumulative
        var_table_source['Cluster Sequences'] = var_table_source['Cluster Sequences'].cumsum()
        
        y_t = var_table_source['Total Sequences']
        y_c = var_table_source['Cluster Sequences']
        
        xs = pd.date_range(start = str(list(x)[0]), end = str(list(x)[-1]))
        exp = 44 #42 to 47
        ys_t = csaps(x, y_t, xs, smooth=10**-exp)
        ys_t[ys_t<0] = 0
        
        ys_c = csaps(x, y_c, xs, smooth=10**-exp)
        ys_c[ys_c<0] = 0
        
        #Do necessary week 1 filling
        # yn = [0, ys[0]]
        # xs2 = pd.date_range(start = str(list(x)[0] - timedelta(days=7)), end = str(list(x)[0]))
        # yn = csaps(xs2[[0,-1]], yn, xs2, smooth=10**-exp)
        # yn[yn<0] = 0
        
        
        # ys = np.append(yn,ys[1:])
        # xs = pd.date_range(start = str(list(xs2)[0]), end = str(list(x)[-1]))#np.append(xs2,xs[1:])
        
        yn_t=[0]
        yn_c=[0]
        #change to daily incident
        for i in range(1,len(xs)):
            ys_daily_t = ys_t[i] - ys_t[i-1]
            ys_daily_c = ys_c[i] - ys_c[i-1]
            ...
            yn_t.append(ys_daily_t)
            yn_c.append(ys_daily_c)
            
        temp = np.transpose(np.array([[v]*len(xs),[c]*len(xs),
                                      xs.strftime("%Y-%m-%d").tolist(),yn_c,yn_t]))
        var_prev_N = pd.concat([var_prev_N, pd.DataFrame(temp,columns=['pangoLineage', 'country', 'date', 'count', 'total'])], axis=0)

var_prev_N.to_csv("splined21i_clustered.csv",index=False)
...
var_prev_N = pd.read_csv("splined21i_clustered.csv") 
var_prev_N['date']= pd.to_datetime(var_prev_N['date'])
...
for c in countries_var:
    ...
    dates_totals = list(set(var_totals.date))
    for d in dates_totals:
        v_d = var_totals[var_totals.date == d]
        v_d = v_d.reset_index(drop=True)
        varList = list(set(v_d.pangoLineage))
        if sum(pd.to_numeric(v_d['count'])) == 0:
               continue
        if len(varList)==1:
            ...
            var_prev_S = var_prev_S.append({'pangoLineage' : varList[0],
                                        'country' : c, 'date' : d,'count':sum(pd.to_numeric(v_d['count'])), 'prev': prev, 
                                        'days_to_prev': days_to_prev, 'pangoLineage2': "N/A", 'prevsO':1},ignore_index = True)
            continue
        
        total = sum(pd.to_numeric(v_d['count'])) #np.mean(v_d.total)#
        vds = v_d.sort_values('count')
        v1 = vds['pangoLineage'].iloc[-1]
        v2 = vds['pangoLineage'].iloc[-2]
        m = list(pd.to_numeric(vds['count']))[-1]/total #max(pd.to_numeric(v_d['count']))
        m2 = list(pd.to_numeric(vds['count']))[-2]/total
        for var in varList:           
            v_c_v = v_d[v_d.pangoLineage == var]
            #prevailance
            prev =  sum(pd.to_numeric(v_c_v['count']))/total
            prevs = prev
            ...
            if prev == 1:
                ...
                if sum(pd.to_numeric(v_c_v['count']))/total == m:
                    prev = sum(pd.to_numeric(v_c_v['count']))/(total*m2)
                    var_prev_S = var_prev_S.append({'pangoLineage' : var,
                                                'country' : c, 'date' : d,'count': sum(pd.to_numeric(v_c_v['count'])), 'prev': prev, 
                                                'days_to_prev': None, 'pangoLineage2': v2, 'prevsO':prevs},ignore_index = True)
                else:
                    prev =  sum(pd.to_numeric(v_c_v['count']))/(total*m) #total 
                    var_prev_S = var_prev_S.append({'pangoLineage' : var,
                                                'country' : c, 'date' : d,'count': sum(pd.to_numeric(v_c_v['count'])), 'prev': prev, 
                                                'days_to_prev': None, 'pangoLineage2': v1},ignore_index = True)
                # prev = prev/(1-prev)
            
            

var_prev_S.to_csv("splined_prevT21i_clusteredA.csv",index=False)
...
var_prev_S.to_csv("splined_prevT21i_clusteredFA.csv",index=False)   