...
def get_dominant_variants_s_valdict(df, shouldFilter=False):
    # returns a dataframe which among others, has a column thats a dict like country: s_value
    ...
    new_df_data = {
        'date': [],
        'pangoLineage': [],
        'country_s_values': [],  # Dictionary of countries and their S values
        'list_of_prev': []  # List of prevalence values
    }

    # Populate the new DataFrame
    for (pangoLineage, date), group in grouped_s_values:
        country_s_values_dict = dict(zip(group['country'], group['S']))
        prevalences = group['prev'].tolist()  # Get the list of prevalences
        new_df_data['date'].append(date)
        new_df_data['pangoLineage'].append(pangoLineage)
        new_df_data['country_s_values'].append(country_s_values_dict)
        new_df_data['list_of_prev'].append(prevalences)  # Add the list of prevalences
