...
from models import EarlyStopper, ModelM, EncodedGCN
#%% 
...
ITERATION_NAME = "embVarReg" if reg_bool else "embVar"
...
for variant in variant_names:
    ...
    for d in dates: # retrospective dates
        try:
            ...
            torch.cuda.empty_cache()
            if use_S:
              model = EncodedGCN(node_features = T+1).to(device)

            else:
              model = EncodedGCN(node_features = T).to(device)
            ...
            start_weights = train(T,max_epochs,optimizer,early_stopper, weight_pos,train_edges,val_edges, svalues_train, svalues_val, reg_bool)
