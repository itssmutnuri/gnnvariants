...
#adj_mat = np.ones(adj_mat.shape) #MAKES FULLY CONNECTED
#%%
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for time, snapshot in enumerate(train_dataset):
            ...
            if reg:
              ...
              mask2 = mask & (~np.isnan(median_per_target)) 
              
              if np.sum(mask2) != 0:             
                cost = cost + (1-p)*torch.mean((y_hat[mask].squeeze()-snapshot.y[mask,0].to(device))**2) + p*(torch.mean((torch.abs(y_hat[mask2].squeeze()-snapshot.y[mask2,0].to(device)) - torch.tensor(median_per_target[mask2]).to(device))**2))
              else:
        ...
        with torch.no_grad():
            for time, snapshot in enumerate(val_dataset):
                ...
                if reg:
                  ...
                  #print(mask2)
                  if np.sum(mask2) != 0:
                    val_cost = val_cost + (1-p)*torch.mean((y_hat[mask].squeeze()-snapshot.y[mask,0].to(device))**2) + p*(torch.mean((torch.abs(y_hat[mask2].squeeze()-snapshot.y[mask2,0].to(device)) - torch.tensor(median_per_target[mask2]).to(device))**2))
                  else:
...
ITERATION_NAME = "GNN_LOSS-0.001"

PARENT_FOLDER = "Results"
SUB_FOLDER = f"{ITERATION_NAME}"
...
for variant in variant_names:
    ...
    mat_contents = scipy.io.loadmat(f'./baseline_new/{variant}_X.mat')

    # Access the "full_res" variable
    full_res = mat_contents['full_res']
    if len(full_res) == 0:
      nan_array = np.full((len(countries), len(countries)), np.nan)
    else:
      full_res = full_res[0]
    ...
    for d in dates: # retrospective dates
        try:
            if len(full_res) != 0:
              time_res = full_res[count][0]
            else:
              time_res = nan_array
            
            
            #Top performing for next time step
            if count == 0:
              median_per_target = np.nanmedian(time_res, axis=0)
            else:
              indices_per_target = result_matrix.T

              # Create a 3x87 matrix by indexing 'error_matrix' using the extracted indices
              error_submatrix = time_res[indices_per_target, np.arange(len(time_res))]
              
              # Calculate the median for each target country
              median_per_target = np.nanmedian(error_submatrix, axis=0)


            top_performing_indices = np.argpartition(time_res, 3, axis=0)[:3]
            result_matrix = top_performing_indices.T
            count+=1
